...
dance <- function(name){
  ...
  colnames(custom_dance) <- c("Playlist", "Song", "Artist", "Album",
                              "Dance Level", "Energy Level")
...
chill <- function(name){
  custom_chill <- get_user_audio_features(name) %>%
    ...
    select(playlist_name, track_name, artist_name, album_name,
           energy, acousticness) %>%
    arrange(energy)
  colnames(custom_chill) <- c("Playlist", "Song", "Artist", "Album",
                              "Energy Level", "Acousticness")
...
simp <- function(name){
  custom_simp <- get_user_audio_features(name) %>%
    ...
    select(playlist_name, track_name, artist_name, album_name,
           energy, valence) %>%
    arrange(valence)
  colnames(custom_simp) <- c("Playlist", "Song", "Artist", "Album",
